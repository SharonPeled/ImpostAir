...
class PaddingTransform:
    def __init__(self, objective, type_, value, patch_len):
        """
        Initializes the padding transform with the provided type and value.
        
        Args:
            objective (str): The objective of the padding.
            type_ (str): The type of padding to apply.
            value (float): The value to use for padding.
        ...
        if type_ != "constant":
            raise NotImplementedError("Only constant padding is implemented yet")
        self.value = value
    ...
    def __call__(self, sample):
        ...
        if T % self.patch_len != 0:
            ...
            ts = F.pad(ts, (0, 0, pad_size, 0), mode='constant', value=self.value)
